...
'''By James C. Hu
This script will:
1) Orchestrate the RVP workflow
2) Perform analysis on the RVP output data
3) Concatenate the new data with the existing archive file.
'''
...
sequencer input field in the input file is case sensitive. Must be "Callisto" or "Ganymede" to reflect sequencer directory path.
'''

infile = pd.read_csv('RVP_main_input.csv').copy()
run = infile['run'].to_list()[0]
...
norm_counts_groups is used in the sum_groups funciton.
'''
...
The main scrip will the following:
...
3) Perform R analysis/normaization/data merging to database files in the metadata directory.
    a) Most common errors in this section is caused by delimiter missmatch for .txt files when performing R analysis.
       Adjusting the delimiter (sep="," to sep="\t", sep="," is default : line 17 of the R script) as needed to fix R analysis related errors.
...
def run_RVP_r_script(run: str) -> None:
    '''
    This function exicutes the R script and moves the output files to the metadata directory.
    '''
    os.chdir(f'/mnt/storage/wastewater_analysis/respiratory_virus/NextSeq1000_{run}/r_analysis')
...
def sort_groups(infile: str) -> pd.DataFrame:
    '''
    This funciton reformats the infile based on the virus_dict.
    '''
...
def sort_df_data(df: pd.DataFrame) -> pd.DataFrame:
    '''
    This function converts datetime in the most tedious way possible.
    Refactor.
    '''
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    '''
    This function removes non RVP samples from the df.
    '''
...
def group_RVP_outputs(infile: str) -> None:
    '''
    This function exicutes other helper functions sequentially to generate the RVP streamlit input file.
    '''
    os.chdir('/mnt/storage/wastewater_analysis/metadata')
...
# Comment out code blow during troubleshooting
#_________________________#
